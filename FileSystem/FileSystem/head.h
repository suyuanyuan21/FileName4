...
const int DATA_BLOCK_LENGTH = 512 ;  // 数据块内容大小(数据块即为一组数据)
...
const string root_path = "C:\Users\苏源源\Documents\大三下\操作系统\新建文件夹\MyFileSystem";
...
typedef struct fileSystem      //文件系统
{
	...
	int free_list[INDEX_LIST_NUMBER][INDEX_LIST_LENGTH];  // 成组链数组  使用的时候从小到大   被使用了置成-1
	//  即，从free_list[0][127]开始，用到freelist[0][0]，用完再free_list[1][127]。。。。最后是free_list[3][0]
